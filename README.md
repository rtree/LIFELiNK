...
- **Time to first success:** Not instrumented as an elapsed duration; we will not invent one. The real-device production success and proof-to-Firebase claim transition are recorded in [task history](doc/tasks.md).
...
To reproduce the environment rather than imitate the UI, start with [host and build setup](doc/host-setup.md), [MVP 0.1 environment/secret names and test checklist](reference/mvp0.1.md), and the [current handover and deployment constraints](reference/handover.md). These documents describe an existing configured GCP project, physical Android device, World ID production RP, and external Twilio/Discord accounts; the repository alone does not supply their credentials. Do **not** create a replacement World ID RP or commit secret values to get a demo running.
