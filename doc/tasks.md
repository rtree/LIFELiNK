...
| P0-14a | IN PROGRESS | ＋Beaconのボタン1/2・短押し/長押しと広告UUID/Major/Minorの対応をドライランで実測し、発信トリガーにする識別値を確定する | P0-14 | 2026-09-26: 実測で Beacon0=待機、Beacon1/2=ボタン1/2、Major bit14=長押しと確定（`doc/plan.md` 6b 章）。旧トリガーが待機広告だった不具合を修正し、長押し bit のみで発信候補にする実装を Samsung 実機へ導入済み。残り: ドライランで短押し→候補なし・長押し→候補1回を実機確認し、その後ドライランOFFで実通話1回を再確認。追記: 利用者ごとのスロット登録（未リンク時はトリガー無効）、PendingIntent LOW_LATENCY 化、反応速度の分解計測ログ（選択/一括コピー・logcat `LIFELiNK.BeaconLog`）、表示を iBeacon/XIAO のみに絞る変更を実機へ導入済み。**既存リンクは旧形式のため再リンクが必要** |
