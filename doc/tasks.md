...
| P2-15 | IN PROGRESS | 最低限の実ダイヤラー機能（約2画面＋α）と画面の実験 SOS を実装・実機確認する（E4） | P2-14 | 2026-09-26 19:55 実装・実機導入（backend rev `00032-tpb` で `contact_phone` 返却）。`DialerActivity`（ACTION_DIAL）、`InCallActivity`（着信・通話中、ロック上表示）、`LifeLinkInCallService`（通知・フルスクリーン着信）、`ConferenceSosOrchestrator`（AI→join 確認→連絡先→**60 秒で打ち切り AI の保留解除**→統合）、Settings「Phone app (experimental)」の role 取得／変更ボタン。role 未保持時は従来の手動経路。戻し方: Settings で電話アプリを Samsung に戻す（`cmd role get-role-holders android.app.role.DIALER` で確認）。残り: role 取得・通常着信／発信・自動会議の実機確認 | P2-14 | role 取得／返却、番号入力／折返し、着信／通話 UI、発信・追加・統合・終了が動く。通常通話を壊さず callback で統合成功を判定。旧 Home SOS を保持。**連絡先を 60 秒呼び出して出なければ発信をやめ、AI レグの保留を解除する**（plan の切断方針）。統合タイミングを backend 経由で AI に伝えると話者推定が改善する |
