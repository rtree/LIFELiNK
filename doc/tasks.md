...
| P0-04 | IN PROGRESS | Firebase Android app、Authentication、`google-services.json` を構成する | P0-02 | Google providerとOAuth clientの組み込み・debug buildは完了。残りは実端末からのログイン成功 |
...
現時点（2026-09-25）で唯一の主線ボトルネックは、人間の対応待ちで `BLOCKED` になっている 2 件である。それ以外の P0 タスクはこの 2 件の後工程として直接・間接に依存するため、最短経路はこの 2 件を早く解消することに尽きる。

1. **P0-04 のブロック解除**: Firebase Authentication の Google provider を有効化し、OAuth 同意画面のサポートメールを選択する（人間の操作）。解除され次第、Android 側の実ログイン確認・P0-05（位置）・P0-06（連絡先）・P0-09（Safety gate）が並行して進められる。
2. **P0-07 のブロック解除**: Twilio account、発信番号、テスト受電番号の準備と、テスト発信先の事前同意（人間の契約・同意）。解除されない限り P0-10（冪等発信）以降は縦断確認ができない。
3. 上記 2 件の解除待ちの間も、P0-08（OpenAI Secret 割り当て）、Android/backend の骨格実装（コードレベル）は並行して進めてよい。ただし「実端末でのログイン成功」「実着信」を要求する完了条件は、対応するブロック解除まで `DONE` にしない。
