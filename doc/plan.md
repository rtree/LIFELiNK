...
2026-09-26 にフル UI モック（`doc/uimock/` と https://pratana-lab.blush.jp/2026tokyo/mock-en/ ）が提供され、最終系から逆算した画面インベントリと未決の論点を 4a 章にまとめた。本節の内容は 4a 章に統合され、以後は 4a 章を正とする。

## 4a. フル UI 画面遷移（最終系からの逆算、UI モック準拠、設計を先に確定・実装は P1 以降）

参照元: `doc/uimock/`（20 画面のスクリーンショット）、https://pratana-lab.blush.jp/2026tokyo/mock-en/ 。ハッカソンでは全部品が動く前に UI 全体の完成形を先に見る価値があるという方針（ユーザー指示）に基づき、主線を止めずに画面遷移とデータ要件を先に確定する。
...
| 3-2 | アラート段階 1（送信時） | 画面点灯＋ローカル音声で「緊急警報。警察へ通報中。位置情報を記録済み」 | P1 | **新規機能**: 端末ローカルの送信時アナウンス。8 章の「AI が相手に話す内容」とは別物 |
| 3-3 | 送信中ステータス | 位置取得・通報・メンバー通知・録音共有準備のチェックリスト | P0/P1 | 既存の Safety gate → backend 呼び出し順序と一致。UI 化のみ必要 |
| 3-4 | アラート段階 2（接続時） | 通話が実際につながった時だけローカル音声で「通報が完了しました」 | P1 | **新規機能**: Twilio `status: in-progress`/`answered` callback を購読し、実接続時のみ発火させる |
| 3-5 | 並行ステータス（AI 通話＋メンバー通知） | 通話タイマー、Discord 通知配信状況を並列表示 | P1 | 8a 章の `emergencySessions.status` 遷移を UI 化したもの |
| 3-6 | メンバー側 Discord 通知 | 位置、座標、最寄り警察署（名称・電話）、地図、「Call ... Police」「Open map」ボタン | P1 | **未決（Option B前提）**: 最寄り警察署の解決は新規の逆ジオコーディング＋警察署データベースが必要。実装は「本物の緊急番号へ自動発信しない」制約と衝突するため 13 章の論点 1 を参照 |
...
1. **「警察に通報する」という文言・実装をどこまで実現するか**: モックの謳い文句は "an AI calls the police on your behalf" であり、3-6 では「最寄りの警察署に電話」ボタンまである。一方 1 章・12 章は「公的緊急通報の代替ではなく、警察・消防・救急への自動発信はしない」という明確な非目標を定めている。ハッカソンで実装するなら、(a) 文言は「警察」を維持しつつ実際の発信先は事前登録・同意済みの番号（デモ用の代替番号や、本人が指定した知人・警備会社等）に限定する、(b) 本当に警察相当機関へ自動発信する仕組みを作る、のどちらかを明示的に選ぶ。(b) は法的・規制的リスクが高く、少なくともハッカソンの実装範囲では (a) を既定とし、UI 文言は「安全のため事前登録済みの連絡先/機関に自動発信します」等へ調整することを推奨する。
...
UI 全体の完成形を先に見る価値はあるが、本節の P1/P2 項目は主線（P0）を止めない。3 つの論点が未決のままでも、P0 の画面・データ設計には影響しない。
