...
- **実測（2026-09-26 10:05〜10:08、画面 ON・ロック解除・ドライラン）**: 3 スロットでリンク成功。短押し 4 回は受理 0。最初の長押しで受理 1 回、パケット→アプリ受信は 11ms（`pending_intent` 約 5ms、`foreground` 約 20ms で PendingIntent 側が先着）。長押し状態への切り替わりはボタン操作と同時にログへ出る。一方、最初の長押し後 60 秒以内の再長押し 3 回（ボタン2長押し、短押しを挟んだボタン1長押し×2）は、長押しパケットが途切れず続いたため「30 秒途切れるまで 1 回」の規則で全て未受理だった。再押下のたびに 60 秒送信が延長される（最後の長押しから 57 秒後に待機へ戻った）。**未決**: 通話終了後などに再度押した場合も再発信させたいなら、判定を「30 秒途切れ」から「長押し状態への切り替わり（別スロットまたは短押し→長押し）」へ変更する案がある。同じボタンの長押し連続は広告上区別できない。二重発信は Safety gate の進行中イベント確認と backend の冪等性で防ぐ。
