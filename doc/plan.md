...
1. **物理ボタンの初期リンクと再登録**: 初期設定フェーズでユーザーが物理ボタン（Beacon または GATT）を自分のアカウントへリンクする。Beacon の場合は advertisement を検知し、自分が保有する個体の識別子（UUID/Major/Minor）を正確に登録する。GATT の場合はスキャン→接続→（必要なら bonding）→サービス探索という「ペアリングに相当する」手続きを行う。どちらも後から解除・再登録できる。
   - **現状の不足**: 6b 章は UUID/Major を製品全体で固定し、Minor を「個体識別の初号機」として決め打ちしているだけで、**ユーザーごとに自分の物理ボタンを検知・登録する画面や API、再登録フローを設計していない**。Beacon は BLE advertisement を受信可能な範囲の全員に見えるため、Minor をユーザーに紐付けて `users/{uid}` 側に保存しないと、近くにいる別の LIFELiNK ユーザーの物理ボタンにも反応してしまう（誤発報の実害がある不足）。GATT は接続が 1 対 1 なので誤発報のリスクはないが、どの端末に再接続すべきかを保存する仕組みが未設計。次回の API/データ設計で `users/{uid}.linked_button: { type: beacon | gatt, uuid, major, minor, device_address, linked_at }` 相当のフィールドと、スキャン→選択→登録の UI（4a 章の画面遷移に追加）を起こす。
...
4. **友人への通知経路**: 緊急コールが発生したとき、友人にも Discord またはアプリ内共有で連絡できる。
   - **現状の対応**: 4a 章・6a 章・Discord 個別連絡候補設計で対応済み。P1-01 で最終方式を決定する。
...
6. **友人リンクの手段は 2 種類**: 友人リンクの理想形は Discord の identity を指定すること。ただし LIFELiNK アプリ同士でのリンクもあり得るため、Google アカウント（のメールアドレス）でもリンクできるようにする。
   - **現状の不足（要修正）**: これまで「友人共有 UI は Option A（LIFELiNK 内 `friend_links`）か Option B（Discord bot）のどちらかを選ぶ」という二択として設計・議論してきたが、実際は **両方を同時にサポートする**（友人が Discord identity で繋がる場合と、相手も LIFELiNK ユーザーで Google アカウントで繋がる場合の両方があり得る）という要求だと分かった。次回のデータ設計では「友人リンク」を `link_type: lifelink | discord` を持つ単一の概念に一般化し、`friend_links`（LIFELiNK 間、uid ベース）と `discordContacts`（Discord ベース）を同じ上位概念の 2 つの実装として整理し直す。P1-01 の「Option A か B か」という決定の枠組みも「両方サポートする前提でどちらから作るか」に修正が必要。
...
上記の不整合・不足点（1 の物理ボタン誤発報リスク、3 の `accuracy_m` 復活、6 の友人リンク二本立て化）は、次回の API・データ設計の見直しで反映する。今回はユースケースの共有と現状把握までとし、スキーマ自体はまだ変更しない。
