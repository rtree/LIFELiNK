...
- **切断方針（2026-09-26 人間決定）**: 会議の親は Galaxy なので、本人が切れば全レッグが終わる。切らないならそれは緊急通話なので、
  無音や保留での自動切断はしない。例外は 2 つだけ: (1) AI レッグは 60 分で backend が Stream を閉じて退出する
  （人間同士のキャリア通話は切れない・切らない。rev `00031-dfn` 実装済み）。(2) 連絡先を 60 秒呼び出して出なければ発信をやめる。
  (2) は標準ダイアラーでは制御できないため P2-15 の自作ダイアラー（`InCallService` で DIALING が 60 秒続いたら `disconnect()`）で実装する。
  留守番電話が応答するとそのまま統合され得る点に注意。
